...
function sanitizeRecipeHtml(dirtyHtml: string): string {
...
export default function RecipeIntelligence() {
  ...
  //  Card click: autofill sample prompt only 
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiMsg: ChatMessage = {
        id: Date.now() + 1,
        type: "ai",
        html: sanitizeRecipeHtml(response.html_response),
      }
            ...
            {messages.map((message) => (
              ...
              >
                ...
                >
                  {message.html ? (
                    <div
                      className={cn(styles.recipeHtml, "whitespace-normal")}
